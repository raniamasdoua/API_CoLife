...
public final class ActivitySubscriptionPolicy {
    ...
    /**
     * Désinscription possible uniquement tant que l'activité n'a pas commencé.
     */
    public static void validateActivityAllowsUnsubscribe(Activity activity, LocalDate today, LocalTime now) {
        LocalDate activityDate = activity.getDate();
        LocalTime activityStart = activity.getStartTime();

        if (activityDate.isBefore(today)) {
            throw new BusinessException("Impossible de se désinscrire d'une activité déjà terminée");
        }
        if (activityDate.isEqual(today) && !now.isBefore(activityStart)) {
            throw new BusinessException("Impossible de se désinscrire d'une activité déjà commencée ou terminée");
        }
    }
}
